...
{
  "id" : "JournalFutureDigitalStatusDateValidate",
  "type" : "BusinessAction",
  "setupGroups" : [ "JournalMediaUpsertGroup" ],
  "name" : "Journal Future Digital Status Effect Date Validate",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
var journalEffectiveDate = NODE.getValue("JournalFutureDigitalStatusEffectDate").getSimpleValue();
...
if (journalEffectiveDate <= today) {
	...
	var emailSubject = "Digital Future Date reached for journal " + productTitle;
	var emailBody = "The Journal " + productTitle + " has reached it Effective Date.\nJorunal Digital Status changed from  "
                   + NODE.getValue("ProductStatus").getSimpleValue() + " to "
                   + NODE.getValue("JournalMediaFutureDigitalStatus").getSimpleValue() + "\nDate " + today
                   + " <= " + journalEffectiveDate;
	
	NODE.getValue("JournalFutureDigitalStatusEffectDate").setSimpleValue("");
