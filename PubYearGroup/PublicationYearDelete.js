...
{
  "id" : "PublicationYearDelete",
  "type" : "BusinessAction",
  "setupGroups" : [ "PubYearGroup" ],
  "name" : "Publication Year Delete",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
*/
exports.operation0 = function (UI,YESNOLOV,mgr,groupIssueOIEP,groupIssueOIEPkafka,logger,pubLibrary,volumeLibrary,pubYearLibrary,issueLibrary) {
/*old:
 * 
var selectedNodes = UI.getSelection();

for (var i = 0; i < selectedNodes.size(); i++){
	pubLibrary.deletePubYear(selectedNodes.get(i), YESNOLOV, UI, MANAGER);
}
*/
...
var severity = "ERROR";
var headline = "";
var body = "The selected Publication Year contains a Volume which has an Issue with JPCMS and Original Publication Date populated";
var errorNodes = [];

for (var i = 0; i < selectedNodes.size(); i++) {
    if (pubYearLibrary.pubYearDeleteCheck(selectedNodes.get(i))) {
        var volumeChildren = [];
        volumeChildren = selectedNodes.get(i).getChildren();
        for (var vol = 0; vol < volumeChildren.size(); vol++) {
            var issueChildren = [];
            issueChildren = volumeChildren.get(vol).getChildren();
            ...
            volumeLibrary.deleteVolume(volumeChildren.get(vol));
        }

        pubYearLibrary.deletePubYear(selectedNodes.get(i));
    } else {
...
if(errorNodes !=""){
	headline = "The Following Publication Year(s) can't be deleted. \n" +errorNodes.join("\n");
