...
{
  "id" : "Import_Issues_Manual_V5",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Import Issues Manual_V5",
  "description" : null,
  "scope" : "Global",
...
var issueProductionCode = node.getValue("IDLJournalProductionIssueCode").getSimpleValue();
...
if (pubYearMatch == false){
	...
	//	log.info("Volume Group FTE Pricing" + volumeGroupFTEPricing);
		...
		newIssue.getValue("IssueDoi").setValue(issueDoi);
		newIssue.getValue("JournalProductionIssueCode").setValue(issueProductionCode);
