...
==============================================================================================================================================================================                                                                    
26June2025    Venkata Siva Harish Mattaparthi   HAR01      RPDM-10259  Changed the logic for LastUpdated,LastUpdatedBy Attributes in Journal Network JSON Feed.                                                                
==============================================================================================================================================================================

-------------------------------------------------------------------------------------------------------------------------------
==============================================================================================================================================================================
*/
function initialNodeJSON(inputNode,manager){
	var json = {};
	...
	//json.LastUpdated=inputNode.getRevision().getEditedDate()+'';
	//json.LastUpdatedBy=inputNode.getRevision().getUserID()+'';
	//json.LastUpdated =isoDate()+'';
	//json.LastUpdatedBy =manager.getCurrentUser().getName()+'';
	return json;
}

// Function to pad with leading zero   - HAR01
function pad(n) {
    return n < 10 ? '0' + n : n;
}

function initialNodeJSONWithoutParent(inputNode,manager){
	var json = {};
	json.stepID = inputNode.getID() + "";
	json.stepName = inputNode.getName() + "";
	//json.parentID=inputNode.getParent().getID()+'';
...
function initialNodeJSONJournalNetwork(inputNode,manager){
	var json = {};
	// Parse the string to a Date object   - HAR01
     var lastUpdated1 = new Date(inputNode.getRevision().getEditedDate());  //HAR01
     var lastUpdatedBy1 = inputNode.getRevision().getUserID();  //HAR01
	...
	json.objectTypeName=inputNode.getObjectType().getName()+'';
	json.TenantID = inputNode.getValue("TenantID").getSimpleValue()+'';
    //json.LastUpdated = inputNode.getRevision().getEditedDate() + ''; //HAR01
    // Format the date
    json.LastUpdated =
        lastUpdated1.getUTCFullYear() + '-' +
        pad(lastUpdated1.getUTCMonth() + 1) + '-' +
        pad(lastUpdated1.getUTCDate()) + ' ' +
        pad(lastUpdated1.getUTCHours()) + ':' +
        pad(lastUpdated1.getUTCMinutes()) + ':' +
        pad(lastUpdated1.getUTCSeconds()) + '';   //HAR01

    //json.LastUpdatedBy = inputNode.getRevision().getUserID() + '';  //HAR01
    json.LastUpdatedBy = manager.getUserHome().getUserByID(lastUpdatedBy1).getName() + '';  //HAR01
...
function getReferencesJournalNetworkAsJSON(inputNode, includedReferenceTypes, embed, checkValid,manager){	
	var referencesObj = [];
	var refIter = inputNode.getReferences().asSet().iterator();
	while(refIter.hasNext()){
		refObj = {};
		var ref = refIter.next();
		var refTypeID = ref.getReferenceType().getID()+'';
		if(refTypeID==includedReferenceTypes){
			refObj.referenceType = refTypeID;			
			refObj.referenceValues = getAllValuesAsJSON(ref,manager,checkValid)
			if(embed){
				refObj.target = initialNodeJSONWithoutParent(ref.getTarget());
				//refObj.target.values= getAllValuesAsJSON(ref.getTarget(),manager,checkValid);
			}
			referencesObj.push(refObj);
		}
...
exports.pad = pad
exports.initialNodeJSONWithoutParent = initialNodeJSONWithoutParent
exports.initialNodeJSONJournalNetwork = initialNodeJSONJournalNetwork
...
exports.getReferencesJournalNetworkAsJSON = getReferencesJournalNetworkAsJSON
