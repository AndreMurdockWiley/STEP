# Functional Description: Send Journal Transition Refs

## Overview
This document describes the STIBO STEP business rule implemented in `Actions/Send_Journal_Transition_Refs.js`. It is generated from the embedded business rule metadata and the JavaScript implementation, following the same overall documentation shape as the Ad Hoc Issue Creation template.
...
## Context
- **Rule ID**: `Send_Journal_Transition_Refs`
- **Rule type**: `BusinessAction`
- **Setup group(s)**: `Actions`
- **Scope**: `Global`
- **Valid object types**: `JournalHistoryProducts`, `Journal`
...
## Inputs
### Bound Inputs (Binds)
- **Operation**: `lookupTableHome` (LookupTableHomeBindContract)
- **Operation**: `node` (CurrentObjectBindContract)
- **Operation**: `manager` (ManagerBindContract)
- **Operation**: `JOURNAL_TO_JOURNAL_TRANSITION` (ReferenceTypeBindContract)
- **Operation**: `HISTORY_TO_HISTORY_TRANSITION` (ReferenceTypeBindContract)
- **Operation**: `Test_Trigger_Rule_Journal_Refs_Send` (BusinessActionBindContract)
- **Operation**: `webUI` (WebUiContextBind)
...
- **Source file**: `Actions/Send_Journal_Transition_Refs.js`
