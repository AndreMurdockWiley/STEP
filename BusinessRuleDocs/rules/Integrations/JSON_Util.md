# Functional Description: JSON Util

## Overview
This document describes the STIBO STEP business rule implemented in `Integrations/JSON_Util.js`. It is generated from the embedded business rule metadata and the JavaScript implementation, following the same overall documentation shape as the Ad Hoc Issue Creation template.
...
- **Source file**: `Integrations/JSON_Util.js`
- **Exports detected**: `initialNodeJSON`, `pad`, `initialNodeJSONWithoutParent`, `initialNodeJSONJournalNetwork`, `getClassificationRefAsJSON`, `getParentClassificationNodesJSON`, `getReferencesAsJSON`, `getReferencesJournalNetworkAsJSON`, `retrieveInheritedAttributeLinks`, `addLinkedAttributeIDsToArray`, `isAttributeValidForNode`, `getValidAttributes`, `getAllValuesAsJSON`, `getAttributeValue`, `getChildObjects`, `getDataContainers`, `getDataContainerObjects`, `getContainerAttrValues`, `isoDate`
