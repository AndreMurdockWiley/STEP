# Functional Description: Collections Integrations JSON Utility Library

## Overview
This document describes the STIBO STEP business rule implemented in `Integrations/Collections_Endpoints_JSON_Utility_Lib.js`. It is generated from the embedded business rule metadata and the JavaScript implementation, following the same overall documentation shape as the Ad Hoc Issue Creation template.
...
## Context
- **Rule ID**: `Collections_Endpoints_JSON_Utility_Lib`
...
- **Source file**: `Integrations/Collections_Endpoints_JSON_Utility_Lib.js`
- **Exports detected**: `currentNodeInitialJSON`, `initialNodeJSON`, `getClassificationRefAsJSON`, `getParentClassificationNodesJSON`, `getReferencesAsJSON`, `getReferencesAsJSON_ModifiedComponents`, `clearAllComponentsReferences`, `retrieveInheritedAttributeLinks`, `addLinkedAttributeIDsToArray`, `addLinkedAttributeIDsToArrayIssues`, `addLinkedAttributeIDsToArrayNONJ`, `addLinkedAttributeIDsToArrayBACKFILE`, `addLinkedAttributeIDsToArrayHistory`, `addLinkedAttributeIDsToArrayMJ`, `addLinkedAttributeIDsToArrayMM`, `addLinkedAttributeIDsToArrayCollections`, `isAttributeValidForNode`, `getValidAttributes`, `getAllValuesAsJSON`, `getAttributeValue`, `getChildObjects`, `getDataContainers`, `getDataContainerObjects`, `getContainerAttrValues`, `getReferencesToValuesAsJSON`, `getReferencedByToValuesAsJSON`, `getReferencedByToValuesAsJSONCollection`, `getReferencedByToValuesAsJSONHistory`
