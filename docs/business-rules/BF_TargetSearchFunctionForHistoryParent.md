## BF_TargetSearchFunctionForHistoryParent

- **Rule type**: Function
- **Business area**: Functions
- **Data model object valid to**: All
- **Source file(s)**: `Functions/BF_TargetSearchFunctionForHistoryParent.js`

### Functional description

Target Search Function for History Journal Parent Objects

### Functional logic

- Plugin: QueryBusinessFunction.
- Parameter "FunctionParameterBinds": <?xml version="1.0" encoding="UTF-8"?>
<FunctionParameterBindMap>
  <Bind contractID="StringBindContract" Alias="string" Description="" ParameterClass="null"/>
  <Bind contractID="NodeBindContract" Alias="node" Description="" ParameterClass="null"/>
</FunctionParameterBindMap>

- Parameter "QueryVariables": PD94bWwgdmVyc2lvbj0iMS4wIiBlbmNvZGluZz0iVVRGLTgiPz4KPFF1ZXJ5VmFyaWFibGVzLz4K
- Parameter "SearchParameter": 

### Errors

—

### Usage / trigger

—

### Dependencies / key functions

—

### Traceability

- **Source workbook**: `Stibo STEP Global Business Rules v1.3.xlsx`
- **Sheet**: `Global Business Rules Template `
- **Row(s) (0-based in data block)**: 280
