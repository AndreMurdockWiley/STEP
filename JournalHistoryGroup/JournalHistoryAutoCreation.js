...
{
  "id" : "JournalHistoryAutoCreation",
  "type" : "BusinessAction",
  "setupGroups" : [ "JournalHistoryGroup" ],
  "name" : "Journal History Auto Creation",
  "description" : null,
  "scope" : "Global",
...
for(var i = 0; i < myList.size() ; i++) {
	...
	LOG.info("A Reference of type 'Journal_History_Reference' already exists. Only updating the existing Journal History.");
	
	if (journalMediaCode == "Print"){
		myList.get(0).getTarget().getValue("JournalHistoryISSNPrint").setSimpleValue(NODE.getValue("ProductIssn").getSimpleValue());
		myList.get(0).getTarget().approve();
		
		
	} else {
		myList.get(0).getTarget().getValue("JournalHistoryISSNOnline").setSimpleValue(NODE.getValue("ProductIssn").getSimpleValue());
		myList.get(0).getTarget().approve();
	}	
} else {
	LOG.info("No existing reference. Creating a new Journal History and creating the reference.");
	...
	myJournalHistoryObject.getValue("HistoryOrigin").setValue("Regular Workflow");
	myJournalHistoryObject.getValue("ProductShortTitle").setSimpleValue(parentNode.getValue("ProductShortTitle").getSimpleValue());//Arun 27/012/2025 RPDM-9545
	...
	myJournalHistoryObject.getValue("SoftDelete").setSimpleValue("No");
	
	if (journalMediaCode == "Print"){
		myJournalHistoryObject.getValue("JournalHistoryISSNPrint").setSimpleValue(NODE.getValue("ProductIssn").getSimpleValue());
	} else {
		myJournalHistoryObject.getValue("JournalHistoryISSNOnline").setSimpleValue(NODE.getValue("ProductIssn").getSimpleValue());
	}
